    ...
    export default {
      ...
      methods: {
        deletePost(id) {
          axios.delete("/api/countries/" + id).then(response => {this.$router.push({name:'read_countrie'});
          })
        },
        nameWithLang ({ name}) {
          return `${name}`
        ...
        getPost() {
          axios.get("/api/countries/" + this.postId)
          .then(response => {
            this.$refs.country_name.value = response.data.data.country_name;
            this.boolean = response.data.data.status;
            this.$refs.priority.value = response.data.data.priority;
            this.$refs.iso_code_2.value = response.data.data.iso_code_2;
            this.$refs.iso_code_3.value = response.data.data.iso_code_3;
            this.$refs.format.value = response.data.data.format;
          })
          .catch(error => {
        ...
        save(text) {
          ...
          axios
            .post("/api/countries/"+ this.postId, formData)
            .then(response => {
