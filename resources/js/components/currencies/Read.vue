...
<div id="toolbar" class="toolbar btn-toolbar ">
  <div class="toolbar-action">
    <router-link :to="{ name: 'create_currencies'}">
    ...
    <b-table striped hover responsive :items="items" 
      ...
      <template v-slot:cell(actions)="row">
        <router-link :to="{ name: 'update_currencies', params: { postId : String(row.item.id) } }">
...
<script>
    ...
    export default {
      ...
      methods: {
        getPosts(address) {
          axios.get(address ? address : "/api/currencies").then(response => {
            this.items = response.data.data;
          });
        },
        deletePost(id) {
          axios.delete("/api/currencies/" + id).then(response => this.getPosts())
        },
