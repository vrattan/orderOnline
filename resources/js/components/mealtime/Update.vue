    ...
    export default {
      ...
      methods: {
        deletePost(id) {
          axios.delete("/api/mealtime/" + id).then(response => {this.$router.push({name:'read_meal'});
          })
        },
        nameWithLang ({ name}) {
          return `${name}`
        ...
        getPost() {
          axios.get("/api/mealtime/" + this.postId)
          .then(response => {
            this.$refs.mealtime_name.value = response.data.data.name;
            this.boolean = response.data.data.status;
            this.start_time = (response.data.data.start_time).substr(0,5);
            this.end_time = (response.data.data.end_time).substr(0,5);
          })
          .catch(error => {
        ...
        save(text) {
          ...
          axios
            .post("/api/mealtime/"+ this.postId, formData)
            .then(response => {
